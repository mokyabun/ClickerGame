...
void CGdiButton::DrawButton(Gdiplus::Graphics& graphics)
{
	...
	if (pCurrentImage && pCurrentImage->GetLastStatus() == Gdiplus::Ok)
	{
		...
		graphics.DrawImage(pCurrentImage, 0, 0, rect.Width(), rect.Height());
	}
	else
	{
		// 이미지가 없으면 기본 버튼 그리기
	...
	if (!m_text.IsEmpty())
	{
		...
		Gdiplus::Font font(&fontFamily, m_fontSize, m_fontStyle, Gdiplus::UnitPixel);
		...
		Gdiplus::RectF layoutRect(0.0f, 0.0f, (Gdiplus::REAL)rect.Width(), (Gdiplus::REAL)rect.Height());
		graphics.DrawString(m_text, -1, &font, layoutRect, &stringFormat, &textBrush);
	}
}
...
{
	// offset은 부모 좌표계에서의 위치
	CRect rect;
	GetClientRect(&rect);
	
	// 상태 변경 결과를 Graphics에 반영
	Gdiplus::Graphics* pGraphics = &graphics;
	
	// DrawButton 호출 (기존 로직 재사용)
	// offset 적용을 위해 변환 행렬 사용
	Gdiplus::GraphicsState state = pGraphics->Save();
	pGraphics->TranslateTransform((Gdiplus::REAL)offset.x, (Gdiplus::REAL)offset.y);
