...
void CClickerButton::Draw(Gdiplus::Graphics& graphics, CPoint offset, float deltaTime)
{
	...
	if (pCurrentImage && pCurrentImage->GetLastStatus() == Gdiplus::Ok)
	{
		...
		graphics.DrawImage(pCurrentImage, offsetX, offsetY, scaledWidth, scaledHeight);
	}
	...
	if (!m_text.IsEmpty())
	{
		...
		Gdiplus::RectF layoutRect((Gdiplus::REAL)offsetX, (Gdiplus::REAL)offsetY, 
			(Gdiplus::REAL)scaledWidth, (Gdiplus::REAL)scaledHeight);
